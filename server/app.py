...
def api_refine_outcome():
    ...
    try:
        ...
        prompt = """당신은 Result Chain 프레임워크와 Observable Outcome 설계에 정통한 교육 설계 전문가입니다.
...
**Outcome (학습 성과):** 수업 직후 학습자에게 나타나는 관찰 가능한 변화
- "~를 이해한다"처럼 머릿속에만 있는 표현을 피하세요.
- 대신 관찰 가능한(Observable) 행동으로 표현하세요:
  - "~의 차이를 비교하여 설명할 수 있다"
  - "~상황에서 ~를 적용한 대안을 제시할 수 있다"
  - "~의 사례를 분석하여 ~원리를 도출할 수 있다"

**Impact (장기적 영향):** 이 학습이 궁극적으로 기여하는 더 큰 역량 (1문장)

## 개선 방향

1. **Observable 사례 추가**: 초안의 추상적 개념마다, 학습자가 실제로 관찰하거나 분석할 수 있는 구체적 사례/현상/사물을 추가하세요.
   - 나쁜 예: "입출력 장치를 이해한다" → 관찰 불가
   - 좋은 예: "터치스크린과 물리 버튼의 피드백 차이를 비교하고, AR Glass에서 시선 추적이 기존 입력 방식을 어떻게 대체하는지 분석할 수 있다" → 관찰 가능

2. **Result Chain 구조화**: Outcome(수업 후 바로 보이는 변화)과 Impact(장기 역량)를 구분하세요.

3. **구체적 범위 명시**: 초안에서 암시된 하위 주제, 키워드, 관찰 대상을 명시적으로 나열하세요.
...
{{"refined": "개선된 전체 텍스트. 아래 구조를 따르세요:\n\n[Outcome]\n관찰 가능한 학습 성과 (구체적 사례 포함, 2-4문장)\n\n[Impact]\n장기적 영향 (1문장)", "changes": "무엇을 보강했는지 한 줄 설명"}}""".format(draft)
